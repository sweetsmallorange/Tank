...
from utils.metrics import calc_nse, cacl_rmse, calc_tpe, calc_kge, calc_new
...
def test_full(test_model, decode_mode, test_loader, device, saving_root, only_metrics: bool, date_index=None):
    ...
    new_test = calc_new(obs_rescaled, pred_rescaled)
    test_new_mean = new_test.mean()
    print(f"Testing mean mse: {test_rmse_mean}, mean nse:{test_nse_mean}, mean new:{test_new_mean}")
    ...
    with open(log_file, "wt") as f:
        ...
        f.write(f"test_new_mean:{test_new_mean}\n")
        f.write(f"{','.join(f'nse{i + 1}' for i in range(len(nses_test)))}\n")
        f.write(f"{','.join(str(i) for i in list(nses_test))}\n")
    ...
    return test_rmse_mean, test_nse_mean, test_new_mean
